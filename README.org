...
** Branch Info - R Package
This Branch is encapsulating the R functions in a package. With this,
not only the organization of the functions is better (splited in
multiples files), but is faster to read the library. Also, some
dependencies can be made optional, and dont require to be read all
times.

This branch can be installed with =devtools=.
